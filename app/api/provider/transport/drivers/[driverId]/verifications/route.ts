...
import { upsertDriverVerificationsSchema } from "@/lib/validation/transport-fleet-schemas";
...
export async function PUT(
  ...
  try {
    ...
    const body = upsertDriverVerificationsSchema.parse(await req.json());
    return jsonOk(
      await upsertDriverVerifications(user, orgId, driverId, body.verifications)
    );
  } catch (e) {
