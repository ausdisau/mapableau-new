...
import { createTransportVehicleSchema } from "@/lib/validation/transport-fleet-schemas";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return jsonOk({ vehicles: await listTransportVehicles(user, orgId) });
  } catch (e) {
...
export async function POST(req: Request) {
  ...
  try {
    await requireProviderOrgId(user, orgId);
    const body = createTransportVehicleSchema.parse(await req.json());
    return jsonOk(await createTransportVehicle(user, orgId, body));
  } catch (e) {
    return handleTransportRouteError(e);
  }
}
