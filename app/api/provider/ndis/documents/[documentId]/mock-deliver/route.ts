...
/** POST /api/provider/ndis/documents/[documentId]/mock-deliver */
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    await recordWorkflowTransition({
      ...
      toStatus: "mock_delivered",
      actorUserId: user.id,
      reason: parsed.data.note ?? "Mock delivery (simulation only)",
      metadata: {
        action: "document.mock_deliver",
        simulation: true,
      },
    });

    return jsonNdisOk({
      document: projectBillingDocumentSafe(updated),
      message: "Marked as mock-delivered. This is not a real delivery.",
    });
  } catch (e) {
