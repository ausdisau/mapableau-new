...
import { createProvider, listProviders } from "@/lib/abilitypay/provider-service";
import { createProviderSchema } from "@/types/abilitypay";
...
export async function POST(req: Request) {
  const user = await requireApiSession();
  if (user instanceof Response) return user;
  const denied = requireAbilityPayPermission(user, "abilitypay:admin");
  if (denied) return denied;
