...
import { applyPilotChangeRequest } from "@/lib/pilot/change/change-request-service";
import { prisma } from "@/lib/prisma";

const bodySchema = z.object({
  deploymentNote: z.string().max(2000).optional(),
});

type Params = { params: Promise<{ pilotId: string; changeId: string }> };

/** POST /api/admin/pilots/[pilotId]/changes/[changeId]/record-deployment */
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    const change = await applyPilotChangeRequest({
    ...
    return jsonNdisOk({
      change: {
        ...
        deploymentNote: parsed.data.deploymentNote ?? null,
      },
    });
  } catch (e) {
