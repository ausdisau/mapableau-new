...
export async function POST(req: Request, context: RouteContext) {
  ...
  const parsed = requestInfoHumanOpsSchema.safeParse(body);
  if (!parsed.success) return zodErrorResponse(parsed.error);

  const operator = await resolveHumanOpsOperatorContext(user);
  const result = requestHumanOpsInformation({
    reviewId,
    operator,
    informationRequested: parsed.data.informationRequested,
    fromParticipant: parsed.data.fromParticipant,
  });
  if (!result.ok) {
    const status =
