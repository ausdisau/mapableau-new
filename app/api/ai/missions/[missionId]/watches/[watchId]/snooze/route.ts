...
export async function POST(req: Request, context: RouteContext) {
  ...
  if (!checkIpRateLimit(`ai-missions-watches-snooze:${ip}`, { windowMs: 60_000, max: 30 })) {
  ...
  const parsed = snoozeWatchBodySchema.safeParse(body ?? {});
  if (!parsed.success) return zodErrorResponse(parsed.error);
  try {
    const watch = snoozeWatch({ missionId, watchId, minutes: parsed.data.minutes });
    await createAuditEvent({
      actorUserId: user.id, participantId: user.id, action: "mission.watch.snoozed",
      entityType: "MissionWatch", entityId: watchId,
      metadata: { missionId, minutes: parsed.data.minutes },
    });
    return jsonOk({ watch });
  } catch (err) {
    return jsonError(err instanceof Error ? err.message : "WATCH_SNOOZE_FAILED", 400);
  }
}
