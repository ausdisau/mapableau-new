...
export default async function ProviderServiceAgreementsPage() {
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Manage agreement lifecycle, activation, and compliance checks.
        </p>
      </header>

      {agreements.length === 0 ? (
        <p role="status" className="text-sm text-muted-foreground">
          No service agreements for your organisation yet.
        </p>
      ) : (
        <ul className="space-y-3">
          {agreements.map((agreement) => (
            <li key={agreement.id}>
              <Link
                href={`/provider/service-agreements/${agreement.id}`}
