...
export default async function ParticipantServiceAgreementDetailPage({
  params,
}: {
  ...
  const agreement = await prisma.serviceAgreement.findFirst({
    where: { id: agreementId, participantId: user.id },
  });
  ...
  return (
    <div className="space-y-6">
      <p>
        <Link
          href="/dashboard/service-agreements"
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="mt-3">
          <AgreementLifecycleActions
            ...
            canParticipantSign={!agreement.participantSignedAt && agreement.status !== "cancelled" && agreement.status !== "expired"}
