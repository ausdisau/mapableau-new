...
import { createAgentPreflightContract } from "@/lib/platform/convergence-os/agent/preflight";
import { seedCounterfactualSimulations } from "@/lib/platform/convergence-os/blast/simulator";
...
import {
  ...
  seedSupplyChain,
} from "@/lib/platform/convergence-os/ops/drift-parity-supply";
import {
  seedOwnershipRegistry,
  seedFitnessFunctions,
  seedGoldenJourneys,
  seedFederation,
} from "@/lib/platform/convergence-os/ops/ownership-fitness-federation";
import { runFoundationTrainRehearsal } from "@/lib/platform/convergence-os/rehearsal/lab";
import { runTwinScan } from "@/lib/platform/convergence-os/scans/twin-scan";
import { seedSemanticCandidates } from "@/lib/platform/convergence-os/semantic/resolver";
...
export async function seedIteration2(options?: {
  ...
  const preflight = await createAgentPreflightContract({
    ...
    allowedPaths: ["lib/platform/convergence-os/", "app/admin/convergence/", "docs/convergence-os/"],
