...
export function mergeGtfsIndexes(indexes: GtfsIndex[]): GtfsIndex {
  const stops = new Map<string, GtfsStop>();
  const routes = new Map<string, GtfsRoute>();
  const trips = new Map<string, GtfsTrip>();
  const stopTimesByStop = new Map<string, GtfsStopTime[]>();
  ...
  for (const index of indexes) {
    for (const [id, stop] of index.stops) stops.set(id, stop);
    for (const [id, route] of index.routes) routes.set(id, route);
    for (const [id, trip] of index.trips) trips.set(id, trip);
    for (const [stopId, times] of index.stopTimesByStop) {
      const existing = stopTimesByStop.get(stopId) ?? [];
      stopTimesByStop.set(stopId, existing.concat(times));
    }
  }
