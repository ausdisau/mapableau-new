...
export function isAutomationEventAllowed(
...
export function isIntegrationEnvEnabled(key: string): boolean {
  const realtimeProvider =
    process.env.NEXT_PUBLIC_REALTIME_PROVIDER ?? process.env.REALTIME_PROVIDER;
  const envMap: Record<string, string | undefined> = {
    ...
    supabase_realtime:
      process.env.SUPABASE_REALTIME_ENABLED ??
      (realtimeProvider === "supabase" ? "true" : undefined),
  ...
  if (key === "supabase_realtime") {
    return val === "true" || realtimeProvider === "supabase";
  }
  return val === "true";
}
