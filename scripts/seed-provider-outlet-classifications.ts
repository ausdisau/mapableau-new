...
async function seedFromDb(limit: number | null, batch: number): Promise<void> {
  ...
  while (processed < target) {
    ...
    const updates = rows.map((row) => {
      const outlet = outletFromRegistryRow({ ...row, raw: null });
      if (!outlet) {
        skipped += 1;
        return {
          id: row.id,
          supportTypes: [] as string[],
          accessNeedIds: [] as string[],
        };
      }
    ...
    const payload = JSON.stringify(
      updates.map((u) => ({
        id: u.id,
        support_types: u.supportTypes,
        access_need_ids: u.accessNeedIds,
      })),
    );

    await prisma.$executeRaw`
      UPDATE provider_outlets AS po
      SET
        support_types = ARRAY(SELECT jsonb_array_elements_text(v.support_types)),
        access_need_ids = ARRAY(SELECT jsonb_array_elements_text(v.access_need_ids)),
        updated_at = NOW()
      FROM json_to_recordset(${payload}::json) AS v(
        id text,
        support_types jsonb,
        access_need_ids jsonb
      )
      WHERE po.id = v.id
    `;
    ...
    if (processed % 1000 === 0 || processed >= target) {
