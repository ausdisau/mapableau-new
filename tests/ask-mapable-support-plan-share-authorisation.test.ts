...
function validInput(
  overrides: Partial<SupportPlanShareAuthorisationInput> = {},
): SupportPlanShareAuthorisationInput {
  return {
...
describe("support-plan sharing authorisation", () => {
  it("builds one-time platform consent from metadata only", () => {
    const result = buildSupportPlanShareAuthorisationConsent(validInput(), NOW);
  ...
  it("never carries support-plan free text into the consent metadata", () => {
    ...
    } as SupportPlanShareAuthorisationInput & {
      sections: Array<{ label: string; value: string }>;
    };

    const result = buildSupportPlanShareAuthorisationConsent(input, NOW);
  ...
  it("rejects a typed external recipient because a label is not a verified identity", () => {
    const result = buildSupportPlanShareAuthorisationConsent(
      validInput({ recipientKind: "external_service" }),
      NOW,
    );
  ...
  it("rejects expired authorisation attempts", () => {
    const result = buildSupportPlanShareAuthorisationConsent(
      validInput({ expiresAt: "2026-09-12T05:29:59.000Z" }),
      NOW,
    );
  ...
  it("rejects an expiry more than seven days away", () => {
    const result = buildSupportPlanShareAuthorisationConsent(
      validInput({ expiresAt: "2026-09-20T05:30:00.000Z" }),
      NOW,
    );
  ...
  it("rejects unknown or empty selected fields instead of broadening consent", () => {
    const unknown = buildSupportPlanShareAuthorisationConsent(
      validInput({ selectedFields: ["communicationAccess", "diagnosis"] }),
      NOW,
    );
    const empty = buildSupportPlanShareAuthorisationConsent(
      validInput({ selectedFields: [] }),
      NOW,
    );
  ...
  it("rejects unknown purposes rather than accepting arbitrary disclosure reasons", () => {
    const result = buildSupportPlanShareAuthorisationConsent(
      validInput({ purpose: "anything_goes" }),
      NOW,
    );
