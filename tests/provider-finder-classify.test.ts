...
describe("classifyProviderOutlet", () => {
  it("classifies outlet with support types, funding, categories, and RegGroup", () => {
    ...
    expect(result.regGroups).toEqual([
      {
        index: 4,
...
describe("summarizeClassifications", () => {
  it("aggregates counts by support type and funding", () => {
    ...
    expect(summary.byRegGroup["Therapeutic Supports"]).toBe(1);
    expect(summary.byRegGroupIndex[35]).toBe(1);
    expect(summary.noRegGroup).toBe(0);
  });
});
