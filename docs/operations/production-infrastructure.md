...
| `AI_GATEWAY_API_KEY` or `GOOGLE_GENERATIVE_AI_API_KEY` | If search interpreter enabled | Required for natural-language provider search and Gemini service category classification (`POST /api/search/classify-category`). |
