...
Combines multiple validated signals into a single continuity context before opening or extending a case.

See [`wave-11-impact-and-detection.md`](./wave-11-impact-and-detection.md) and [`wave-11-signal-taxonomy.md`](./wave-11-signal-taxonomy.md).

## Rules

- Correlation is deterministic; idempotency keys exclude `Date.now()`.
- Stale or unvalidated signals are excluded from destructive correlation paths.
- Correlation proposes cases; it does not mutate service status.
