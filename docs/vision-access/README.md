...
## Living Access Fabric integration

VisionAccess candidates bridge into Access Intelligence Next **shadow change reviews** via
`lib/vision-access/ai-next-bridge.ts` and `POST /api/vision-access/shadow-review`.

- Evidence classes: `synthetic_fixture` / `model_candidate` / `device_assisted_estimate`
- Never `professional_measurement` from vision geometry
- Auto-overwrite of verified Twin evidence remains blocked
...
| `POST /api/vision-access/shadow-review` | Fixture candidate → AI-Next shadow review |
