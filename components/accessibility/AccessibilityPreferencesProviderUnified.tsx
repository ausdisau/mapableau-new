/**
 * Unified Accessibility Provider
 *
 * Bridges the new Zustand accessibility store with the existing React Context.
 * Provides both the new store and the legacy context for backward compatibility.
 */
...
} from "@/lib/accessibility/semantic-tokens";
import {
...
/**
 * Legacy context shape (for backward compatibility)
 */
...
/**
 * Legacy hook - use useAccessibilityStore instead
 * @deprecated Use useAccessibilityPreferencesUnified or import from lib/accessibility/index.ts
 */
...
/**
 * Legacy hook - use useAccessibilityStore instead
 * @deprecated Use lib/accessibility/index.ts hooks
 */
...
/**
 * New unified hook - wraps both Zustand store and legacy context
 */
export function useAccessibilityPreferencesUnified(): AccessibilityPreferencesContextValue {
  const ctx = useContext(AccessibilityPreferencesContext);
  if (!ctx) {
    throw new Error(
      "useAccessibilityPreferencesUnified must be used within AccessibilityPreferencesProvider"
    );
  }
  return ctx;
}
...
/**
 * Unified Provider - bridges legacy context and new Zustand store
 */
export function AccessibilityPreferencesProvider({
  ...
  // Initialize the Zustand store on mount
  useAccessibilityInitialization();

  // Get store methods and state
  const store = useAccessibilityStore();
  const preferences = store.uiPreferences;
  const semanticTokens = store.semanticTokens;
  ...
  }, [preferences, semanticTokens, hydrated]);
  ...
  const setPreference = useCallback(
    <K extends keyof AccessibilityUiPreferences>(
      ...
      store.updateUiPreferences({ [key]: value });
