...
export function PublicKnowledgeClient({
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <label
          htmlFor="public-mapable-question"
          className="text-sm font-black text-[#0C1833]"
        >
          Ask a public MapAble question
        </label>
        <p
          id="public-mapable-question-help"
          className="mt-1 text-sm leading-6 text-slate-600"
        >
          Answers are limited to material MapAble has deliberately placed in its
          public knowledge namespace.
        </p>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            type="submit"
            disabled={
              !enabled || status === "loading" || question.trim().length < 3
            }
        ...
        {!enabled ? (
          <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm font-medium text-amber-900">
            The public knowledge guide is wired but not enabled in this
            environment.
          </p>
        ) : null}
        {status === "error" ? (
          <p
            role="alert"
            className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-medium text-red-800"
          >
            {error}
          </p>
        ) : null}
        ...
        >
          ...
          {result.sources.length ? (
            <section className="mt-6">
              <h3 className="text-sm font-black uppercase tracking-[0.14em] text-[#005B7F]">
                Sources used
              </h3>
              <ul className="mt-3 space-y-2">
                {result.sources.map((source) => (
                  <li
                    key={source.key}
                    className="rounded-xl bg-[#F6FBFC] p-3 text-sm text-slate-700"
                  >
                    <code className="font-bold text-[#0C1833]">
                      {source.key}
                    </code>
